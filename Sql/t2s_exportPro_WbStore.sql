    ...
    select 
                ...
                ,convert(xml, 
                            (select pkp.Calc_UNI_Desc as "@package", convert(varchar(8), pkp.Calc_UNI_Conversion_Factor) as "@count"
                                    ...
                                    ,coalesce((select top 1 barcode from barcode_map m where m.pkp_source_id = pkp.pkp_source_id and m.pkp_id = pkp.pkp_id) 
                                                        , 'p:' + convert(varchar(16), pkp.pkp_source_id) + ':' + convert(varchar(16), pkp.pkp_id) )                as "@pkp_upc"
